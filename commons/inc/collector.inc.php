...
/*
	opentime
	$Author: perrick $
	$URL: svn://svn.noparking.net/var/repos/opentime/inc/collector.inc.php $
	$Revision: 5200 $
...
class Collector implements iterator, countable, arrayAccess {
	...
	protected function get_columns() {
		...
		if (sizeof($this->columns) <= 0) {
			$columns[] = "`".$this->db->config['table_'.$this->table]."`.*";
		} else {
	...
	protected function get_from() {
		return array($this->db->config['table_'.$this->table]);
	}
